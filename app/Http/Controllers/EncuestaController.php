...
class EncuestaController extends Controller
{
    ...
    public function grupoIndex()
    {
        ...
        $encuestas = DB::table('encuesta_grupo_im as t1')
            ...
            ->join(
                DB::raw('(SELECT id_empleado,id_encuesta_im,enabled from encuesta_im_empleado where id_empleado=' . $id_empleado . ') t2'),
                't2.id_encuesta_im',
                't3.id'
            )
    ...
    public function index($id_encuesta_grupo_im)
    {
        ...
        for ($i = 0; $i < count($encuestas); $i++) {
            ...
            ifnull(t2.cuenta,0) as cuenta')
                ->join(
                    DB::raw('(select id_empleado,id_encuesta_im,cuenta from encuesta_im_empleado where id_empleado=' . $id_empleado . ')t2'),
                    't2.id_encuesta_im',
                    't1.id'
                )
                ->whereraw('t1.id_company = ' . $id_company)
